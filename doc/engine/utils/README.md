...
> **版本**: 1.0.0  
> **最后更新**: 2026-02-15  
> **相关文档**: [核心模块](../core/README.md) | [ECS 架构](../../ECS_ARCHITECTURE.md#事件系统)

Utils 模块提供了引擎中使用的各种工具类和辅助结构，包括数学工具、枚举定义和事件结构。
...
**文件**: `src/engine/utils/events.h`

定义引擎中使用的事件结构，用于 `entt::dispatcher` 事件系统。
...
|------|------|
| `QuitEvent` | 退出游戏事件 |
| `PopSceneEvent` | 弹出当前场景事件 |
| `PushSceneEvent` | 压入新场景事件 |
| `ReplaceSceneEvent` | 替换场景栈事件 |
| `PlayAnimationEvent` | 播放动画事件 |
| `AnimationFinishedEvent` | 动画播放结束事件 |
...
dispatcher.enqueue<engine::utils::PushSceneEvent>(
    std::make_unique<GameScene>(context)
);

// 发送动画播放事件
dispatcher.trigger<engine::utils::PlayAnimationEvent>(entity, "attack"_hs, false);

// 订阅事件
dispatcher.sink<engine::utils::AnimationFinishedEvent>()
    .connect<&MySystem::onAnimationFinished>(this);
```
